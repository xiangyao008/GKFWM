function [ua_iter,tk_xj,cg_it] = NLFVIE_CWM_Simpson(fx,dvkut,dfkut, n, iter,tol)
...
for i=2:iter+1  
    ...
    kf_new=get_kf(tk_xj_new,tk_xj_new,tk_xj_new2,u,u2_new);
...
ktv1=-t(2:length(x))'.*(u(2:length(x)).^2).*length_int';
    kf_V1=(temp_Volt*ktv+4*temp_Volt*ktv_m+temp_Volt*ktv1).*1/6;
ktv2=(u(1:length(x)-1).^2).*length_int';
ktv2_m=(u2.^2).*length_int';
ktv3=(u(2:length(x)).^2).*length_int';
    kf_V2=(temp_Volt*ktv2+4*temp_Volt*ktv2_m+temp_Volt*ktv3).*(x(2:length(x)))'.*1/6;
    kf_V=kf_V1+kf_V2;
...
ktf3=u(2:length(x)).*length_int';
    kf_F2=(temp_Fred*ktf2+4*temp_Fred*ktf2_m+temp_Fred*ktf3).*(x(2:length(x)))'.*1/6;
    kf_F=kf_F1+kf_F2;
kf=kf_V+kf_F;
end