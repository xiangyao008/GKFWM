function [ua_iter,tk_xj,cg_it] = NLVIE_CWM(fx,dkut, n, iter,tol)
...
% kt=(1./(1+u(1:length(x)-1).^2)).*length_int';
% kt1=(1./(1+u(2:length(x)).^2)).*length_int';
% kf=(temp*kt+temp*kt1)./2;
% end
%% 问题2
% function kf=get_kf(x,t,u)
% length_int=t(2:length(x))-t(1:length(x)-1);
% temp=tril(ones(length(x)-1));
% kt=(t(1:length(x)-1)'.^2).*(u(1:length(x)-1).^2).*length_int';
% kt1=(t(2:length(x))'.^2).*(u(2:length(x)).^2).*length_int';
% kf=(temp*kt+temp*kt1).*x(2:length(x))'./2;
% end
%% 问题3
function kf=get_kf(x,t,u)
length_int=t(2:length(x))-t(1:length(x)-1);
temp=tril(ones(length(x)-1));
kt1=(exp(-2.*t(1:length(x)-1)')).*(u(1:length(x)-1).^2).*length_int';
kt2=(exp(-2.*t(2:length(x))')).*(u(2:length(x)).^2).*length_int';
kf=(temp*kt1+temp*kt2).*exp(-x(2:length(x)))'./2;
end
%% 问题4 Third kind of NLVIE 
% function kf=get_kf(x,t,u)
% length_int=t(2:length(x))-t(1:length(x)-1);
% temp=tril(ones(length(x)-1));
% kt1=(u(1:length(x)-1).^2+u(1:length(x)-1)).*length_int';
% kt2=(u(2:length(x)).^2+u(2:length(x))).*length_int';
% kf=(temp*kt1+temp*kt2).*(-1)./2;
% end